...
const ProductDetailPage = () => {
    ...
    const addToCart = async () => {
        if (!product) return notifyError("Không tìm thấy sản phẩm trong cơ sở dữ liệu. Vui lòng thử lại sau")

        const guestToken = getGuestToken()
        if (!guestToken) return notifyError("Có lỗi xảy ra. Vui lòng thử lại sau")

        const variant = getVariant(chosenColor, chosenSize)
        if (!variant) return notifyError("Không tìm thấy màu sắc và kích thước này trong cơ sở dữ liệu. Vui lòng thử lại")
